...
// 		 }
        ...
        //   if(!($wallet >= 10 && $wallet >=$amount)){
		   if (!($wallet + $winning_wallet >= $amount)) {
					return response()->json(['status' => 400, 'message' => 'Insufficient funds!']);
				}

				// 1️⃣ Pehle winnings check karo
				if ($winning_wallet >= $amount) {
					// Agar winnings se pura amount cover ho jaye, toh sirf winnings se kaato
					DB::table('users')->where('id', $uid)->update([
						'today_turnover' => DB::raw("today_turnover + $amount"),
						'winning_wallet' => DB::raw("winning_wallet - $amount")
					]);
				} else {
					// Agar winnings kam hain, toh winnings ko zero karo aur baaki wallet se kaato
					$remaining = $amount - $winning_wallet;
					DB::table('users')->where('id', $uid)->update([
						'today_turnover' => DB::raw("today_turnover + $amount"),
          ...
          if(!$bet_details){
			...
			if($update){
			  ...
			  $wallet_history =  DB::table('wallet_histories')->insert([
                    'user_id' => $uid,
                    'amount' => $win_amount,
                    'type_id' =>24,   //24 for aviator
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
					
				if($user_update && $wallet_history){
					return response()->json(['message'=>"$win_amount BDT. Cashout Successfully",'status'=>200]);
				}else{
...
{
    $result = DB::table('aviator_result')
        ->orderBy('id', 'desc')
        ->value('game_sr_num');

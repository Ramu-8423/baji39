...
public function bet(Request $request)
{
    ...
    // Check user wallet balance
    if ($user->winning_wallet < $request->amount) {
...
//dd($request->amount);
    ...
    $user->decrement('winning_wallet', $request->amount);
	...
	//$deduct_jili = jilli::deduct_from_wallet($uid,$amount);'jili'=>$deduct_jili
     $recharge = DB::table('users')->where('id', $user->id)->value('recharge'); 
     $new_balance = max(0, $recharge - $request->amount); // Ensure no negative balance
     DB::table('users')->where('id', $user->id)->update(['recharge' => $new_balance]);

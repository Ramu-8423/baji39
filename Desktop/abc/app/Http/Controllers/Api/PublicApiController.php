	...
	public function commission_details(Request $request)
{
    ...
    $userid = $request->userid; // Corrected to match the request field name
    $typeid = $request->typeid; // Corrected to match the request field name
    $date = $request->date;

    // Assuming 'created_at' is correct in the DB
   ...
   public function login(Request $request)
{
    // Validate the input
    $validator = Validator::make($request->all(), [
        'mobile' => 'required|string|regex:/^\+?\d{7,15}$/',
        'password' => 'required'
    ]);
    ...
    // Directly check if the plain text password matches
	...
	//  DB::update("UPDATE `users` SET `login_token` = ? WHERE `id` = ?", [$token, $user->id]);
    ...
    try {
        ...
        if ($user) {
            return response()->json([
                ...
                'apk_link' => env('APP_URL') . "baji39.apk",
                'usdt_payin_amount' => 94,
                'usdt_payout_amount' => 92,
                'telegram' => "https://t.me/baji39official",
                'referral_code_url' => "https://root.baji39.com/registerwithref/" . $user->referral_code,
	...
	public function main_wallet_transfer(Request $request)
{
	...
	if($thirdpartywallet == 0){
		 return response()->json([
            'status' => 400,
            'message' => "No amount available for transfer."
        ],200);
	}
...
//dd($userPercentage,$amount);
    ...
    $payment->status = 2;
    $payment->save();
    // Call the private functions to add to the Zilli and Spribe wallets
    //$account_token = $user->accountNo;
   // $spribe_id = $user->spribe_id;

    //$zili_wallet_resp = $this->addToZiliWallet($userid, $final_amt, $account_token);
    //$spribe_wallet_resp = $this->addToSpribeWallet($userid, $final_amt, $spribe_id);
   ...
   // }
    ...
    // Return failure response if Zilli wallet update fails
    //return response()->json([
     //   'status' => 400,
   //     'message' => 'Error updating wallets.',
   // ], 200);
}
    ...
    if ($amount >= $minAmount && $amount <= $maxAmount) {
        ...
        if ($user_wallet >= $amount) {
            ...
            if ($first_recharge == 0) {
                ...
                DB::select("UPDATE users SET wallet = wallet - y  WHERE id = $userid");
        ...
        if ($giftCart->availed_num < $giftCart->number_people) {
            ...
            if (!$claimUser) {
                ...
                if (!empty($giftCartAmount)) {
                    ...
                    User::where('id', $userid)->increment('winning_wallet', $giftCartAmount);
...
public function updateSpribeIdForUsers()
{
    ...
    if ($validator->fails()) {
		...
		//dd($referral_code, $request->referral_code);
    ...
    try {
        ...
        if ($jilliResponse->successful() && isset($jilliData['accountNo'])) {

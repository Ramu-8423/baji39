...
class BusinessSettingController extends Controller
{
   ...
   public function business_data(){
    $data = DB::table('business_settings')
        ->select('id', 'title', 'longtext') // ✅ id ko include karein
        ->whereIn('id', [15, 16, 17, 18])
        ->get();
    
    return view('BusinessSetting.updatedata', compact('data'));
}
	public function business_update_data(Request $request){
    $longtextData = $request->input('longtext');

    if (!empty($longtextData)) {
        foreach ($longtextData as $id => $value) {
            DB::table('business_settings')
                ->where('id', $id)
                ->update(['longtext' => $value]);
        }
    }
    return redirect()->back()->with('message', 'Business settings updated successfully!');
}
